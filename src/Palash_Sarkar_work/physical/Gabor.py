...
class Gabor(object):
        ...
        def __init__(self, img,masku8, corr_colimg, imtype='color'):
            
            cnts, hier = cv2.findContours(masku8 , cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_NONE)
            self.__contours = cnts
            self.__hierarchy   = hier
            self.__gblurimg = cv2.GaussianBlur(img, (3, 3),sigmaX=0,sigmaY=0)
